...
import { useGetVessels, useGetShipmentsByVessel } from '@/hooks/useQueries';
import { Loader2, Ship, ChevronDown, ChevronUp, Package } from 'lucide-react';
...
function VesselCard({ vessel }: { vessel: any }) {
  ...
  const { data: shipments } = useGetShipmentsByVessel(vessel.id);
  const [isOpen, setIsOpen] = useState(false);

  const activeShipments = shipments?.filter(
    (s) => s.currentStatus === 'pending' || s.currentStatus === 'inTransit'
  );

  return (
    <Card className="group overflow-hidden border-maritime-border bg-maritime-card transition-all hover:shadow-xl">
      <div className="relative h-48 overflow-hidden bg-maritime-navy">
        ...
        {activeShipments && activeShipments.length > 0 && (
          <Badge className="absolute top-2 right-2 bg-maritime-gold text-maritime-navy">
          ...
          <Collapsible open={isOpen} onOpenChange={setIsOpen} className="mt-4">
            <CollapsibleTrigger asChild>
              <Button variant="outline" className="w-full" size="sm">
                ...
                View Shipments ({activeShipments.length})
                {isOpen ? (
            ...
            <CollapsibleContent className="mt-3 space-y-2">
              {activeShipments.map((shipment) => (
                <div
                  key={shipment.id}
                  className="cursor-pointer rounded-lg border border-maritime-border p-3 hover:bg-maritime-section transition-colors"
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <Badge
                      variant={shipment.currentStatus === 'inTransit' ? 'default' : 'secondary'}
                      className="text-xs"
                    >
                      {shipment.currentStatus === 'inTransit' ? 'In Transit' : 'Pending'}
                    </Badge>
                  </div>
                  <p className="text-xs text-maritime-blue-light">
                    {shipment.cargoDetails.length} cargo items
                  </p>
                </div>
              ))}
            </CollapsibleContent>
